...
const STOCK_MOVEMENT_TYPE_CLASSNAME: Record<StockMovementType, string> = {
...
const STOCK_MOVEMENT_TYPE_DOT_CLASSNAME: Record<StockMovementType, string> = {
  PURCHASE: 'bg-[#84994F]',
...
export function StockMovementTypeBadge({ type }: { type: StockMovementType }) {
  return (
    <span className={cn('inline-flex items-center gap-1.5 border px-2 py-0.5 text-xs font-normal', STOCK_MOVEMENT_TYPE_CLASSNAME[type])}>
      <span className={cn('size-1.5 shrink-0 rounded-full', STOCK_MOVEMENT_TYPE_DOT_CLASSNAME[type])} />
