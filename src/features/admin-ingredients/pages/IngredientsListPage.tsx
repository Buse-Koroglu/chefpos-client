...
import { useIngredientsByLocation } from '@/features/admin-ingredients/hooks/useIngredientsByLocation'
import { useFilteredIngredients } from '@/features/admin-ingredients/hooks/useFilteredIngredients'
...
export function IngredientsListPage() {
  ...
  const { data: source = [], isLoading, isFetching, isError, error } = useIngredientsByLocation(locationId, locations)
  const { items, totalCount, totalPages, pageNumber: currentPage } = useFilteredIngredients(
    source,
    searchTerm,
    status,
    pageNumber,
