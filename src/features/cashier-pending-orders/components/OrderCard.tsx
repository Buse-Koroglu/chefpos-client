    ...
    <div className="flex flex-col border-2 border-zinc-300 bg-white">
      ...
      <div className="flex-1 p-4">
        {order.items.length === 0 ? (
          <p className="text-xl text-zinc-500">Ürün bilgisi yok</p>
        ) : (
          <ul className="space-y-1.5 text-xl text-zinc-700">
            {order.items.slice(0, MAX_VISIBLE_ITEMS).map((item) => (
              <li key={item.id} className="flex items-center gap-1.5">
                <span className="shrink-0 tabular-nums text-zinc-500">{item.quantity}x</span>
                <span className="flex-1 truncate">{item.name}</span>
              </li>
            ))}
            {order.items.length > MAX_VISIBLE_ITEMS && (
              <li className="text-zinc-500">... (diğer {order.items.length - MAX_VISIBLE_ITEMS} ürün)</li>
            )}
          </ul>
        )}
      </div>

      <div className="border-t-2 border-zinc-300 p-4">
        <span className="mb-3 block text-3xl font-semibold tabular-nums text-zinc-900">
        ...
        <div className="flex gap-2">
          ...
          {isReadyForPayment ? (
            <Button
              ...
              disabled={isBusy}
              className="h-11 flex-1 rounded-none bg-[#84994F] text-base text-white hover:bg-[#708243]"
            >
              Ödeme Al
            </Button>
          ) : (
            <Button
              ...
              disabled={isBusy}
              className="h-11 flex-1 rounded-none bg-[#133458] text-base text-white hover:bg-[#0f2843]"
            >
