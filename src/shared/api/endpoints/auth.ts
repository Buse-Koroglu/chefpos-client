...
import type { ChangePasswordRequest, LoginRequest, LoginResponse, RawLoginResponse, RefreshResponse } from '@/shared/types/auth'

export function login(payload: LoginRequest) {
  return apiClient.post<RawLoginResponse>('/api/auth/login', payload)
    .then((res): LoginResponse => ({ ...res.data, user: mapUserResponse(res.data.user) }))
}

export function refresh() {
  return apiClient.post<RawLoginResponse>('/api/auth/refresh')
