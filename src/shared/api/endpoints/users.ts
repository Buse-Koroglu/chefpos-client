...
export function getUsers(params: GetUsersQueryRequest): Promise<GetUsersResult> {
  const pageNumber = params.pageNumber ?? 1
  const pageSize = params.pageSize ?? 20

  return apiClient.get< | RawUserPayload[] | { items: RawUserPayload[]; totalCount?: number; pageNumber?: number; pageSize?: number; totalPages?: number }
  >('/api/users', { params })
    .then((res) => {
      const data = res.data
      ...
      const totalPages = Array.isArray(data)? Math.max(1, Math.ceil(totalCount / resolvedPageSize)) : (data.totalPages ?? Math.max(1, Math.ceil(totalCount / resolvedPageSize)))
      return { items, totalCount, pageNumber: resolvedPageNumber, pageSize: resolvedPageSize, totalPages }
    })
}

export function exportUsers(params: ExportUsersQueryRequest): Promise<Blob> {
  return apiClient.get<Blob>('/api/users/export', { params, responseType: 'blob' })
  .then((res) => res.data)
}
...
export function createUser(payload: CreateUserRequest) {
  return apiClient.post<RawUserPayload>('/api/users', {
...
export function assignLocationAccess(userId: string, locationId: string) {
  return apiClient.post<RawUserPayload>(`/api/users/${userId}/locations`, { locationId })
...
export function addRole(userId: string, role: Role) {
  return apiClient.post<RawUserPayload>(`/api/users/${userId}/roles`, { role: ROLES.indexOf(role) })
    .then((res) => normalizeUser(res.data))
}
...
export function getStockManagerByLocation(locationId: string) {
  return apiClient.get<RawUserPayload | null>('/api/users/stock-manager', { params: { locationId } })
    .then((res) => (res.data ? normalizeUser(res.data) : null))
}

export function getAdminByLocation(locationId: string) {
  return apiClient.get<RawUserPayload | null>('/api/users/admin', { params: { locationId } })
    .then((res) => (res.data ? normalizeUser(res.data) : null))
}
