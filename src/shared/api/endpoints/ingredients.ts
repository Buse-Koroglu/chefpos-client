...
export function getIngredients(params: GetIngredientsQueryRequest) {
  return apiClient.get<RawIngredientPayload[]>('/api/ingredients', { params })
    .then((res) => res.data.map(normalizeIngredient))
}

export function getIngredientsPaged(params: GetIngredientsPagedQueryRequest) {
  return apiClient.get<PagedResult<RawIngredientAdminPayload>>('/api/ingredients/paged', { params })
    .then((res) => ({ ...res.data,
      items: res.data.items.map((raw): IngredientAdminResponseDto => ({ ...normalizeIngredient(raw), locationName: raw.locationName })),
    }))
}

export function exportIngredients(params: ExportIngredientsQueryRequest): Promise<Blob> {
  return apiClient.get<Blob>('/api/ingredients/export', { params, responseType: 'blob' })
    .then((res) => res.data)
}

export function createIngredient(payload: CreateIngredientRequest) {
  return apiClient.post<RawIngredientPayload[]>('/api/ingredients', { ...payload, unit: STOCK_UNITS.indexOf(payload.unit) })
    .then((res) => res.data.map(normalizeIngredient))
}
...
export function updateIngredientMinStockThreshold(id: string, minStockThreshold: number) {
  return apiClient.patch<RawIngredientPayload>(`/api/ingredients/${id}/min-stock-threshold`, { minStockThreshold })
...
export function deactivateIngredient(id: string) {
  return apiClient.post<RawIngredientPayload>(`/api/ingredients/${id}/deactivate`)
    .then((res) => normalizeIngredient(res.data))
}

export function recordIngredientPurchase(id: string, payload: IngredientPurchaseRequest) {
  return apiClient.post<RawIngredientPayload>(`/api/ingredients/${id}/purchases`, payload)
    .then((res) => normalizeIngredient(res.data))
}

export function recordManualDeduction(id: string, payload: ManualDeductionRequest) {
  return apiClient.post<RawIngredientPayload>(`/api/ingredients/${id}/manual-deduction`, payload)
    .then((res) => normalizeIngredient(res.data))
}

export function recordProductProduction(payload: ProductProductionRequest) {
  return apiClient.post<RawIngredientPayload[]>('/api/ingredients/production', payload)
    .then((res) => res.data.map(normalizeIngredient))
}
